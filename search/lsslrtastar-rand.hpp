...
template <class D>
class Lsslrtastar2Random : public SearchAlgorithm<D> {
private:
...
public:
	...
	Lsslrtastar2Random(int argc, const char *argv[]) :
		...
		for (int i = 0; i < argc; i++) {
			...
			else if(i < argc - 1 && strcmp(argv[i], "-rand") == 0)
				random_prob = strtod(argv[++i], NULL);
		...
		if(random_prob < 0)
			fatal("please supply a probability for expanding a random node on open");
	...
	virtual void output(FILE *out) {
		...
		dfpair(out, "seed", "%lu", (unsigned long) seed);
		nodes.output(stdout);
...
private:
	...
	LssNode *expandLss(D &d, Node *rootNode) {
		...
		while (!lssOpen.empty() && !lsslim->stop() && !this->limit()) {
			...
			if(rng.real() <= random_prob) {
				int index = rng.real() * lssOpen.size();
				s = lssOpen.at(index);
			}
			else {
				s = lssOpen.pop().get();
