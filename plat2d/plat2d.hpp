...
private:
	...
	Cost hvis(const State &s) const {
		...
		double h = togoal[c].d - Geom2d::Point::distance(loc, vg->verts[c].pt) - diag;
	...
	Geom2d::Point goalpt(const Lvl::Blkinfo &bi, const Geom2d::Point &loc) const {
		Geom2d::Point pt;
		if (bi.y == gy)
			pt.y = loc.y;
		else if (bi.y < gy)
